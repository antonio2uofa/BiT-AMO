...
int main() {
	...
	if(NoOfDetectedUSBDevices > 0) {
		...
		// Set Trigger pin to high (Sensor 3 on schema.)
		CommandRead_0111(&RTDACUSBBuffer);
		RTDACUSBBuffer.CN1Output = 0x1000080;
		CommandSend_0111(&RTDACUSBBuffer);

		// Set Trigger pin to low (Sensor 3 on schema.)
		CommandRead_0111(&RTDACUSBBuffer);
		RTDACUSBBuffer.CN1Output = 0x0000080;
		CommandSend_0111(&RTDACUSBBuffer);

		// Wait for echo pin to go to low state (Sensor 3 on schema.)
		while( input_final != 0) {
			CommandRead_0111(&RTDACUSBBuffer);
		...
		// As soon as sensor goes to low read from counter (Sensor 3 on schema.)
