...
int main() {
	...
	if(NoOfDetectedUSBDevices > 0) {
		...
		// Set Trigger pin to high (Sensor 2 on schema.)
		CommandRead_0111(&RTDACUSBBuffer);
		RTDACUSBBuffer.CN1Output = 0x0800100;
		CommandSend_0111(&RTDACUSBBuffer);

		// Set Trigger pin to low (Sensor 2 on schema.)
		CommandRead_0111(&RTDACUSBBuffer);
		RTDACUSBBuffer.CN1Output = 0x0000100;
		CommandSend_0111(&RTDACUSBBuffer);

		// Wait for echo pin to go to low state (Sensor 2 on schema.)
		while( input_final != 0) {
			CommandRead_0111(&RTDACUSBBuffer);
		...
		// As soon as sensor goes to low read from counter (Sensor 2 on schema.)
